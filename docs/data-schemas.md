...
- `run_mining_screen(*, ohlcv_path, basic_path, moneyflow_path, factors=None, fwd_window=5, min_obs=5, min_days=10, t_active=2.0, layer_t=2.81, output_dir=None) -> dict`：键 `screen`（index=因子，all + 9 层 × {mean_ic,t_stat,n_days} + domain + sign）、`layers`、`summary`（每因子 `{domain, sign}`，sign ∈ {-1, 0, 1}）；output_dir 写 screen.parquet / layers.parquet / summary.json
- 适用域判定（**双侧**）：|全市场 t| ≥ t_active → "universal"；否则 Bonferroni 校正（layer_t=2.81）下 |t| ≥ layer_t 的显著层列表；无 → "none"。负显著不再漏报为 none，方向由 sign 标注（0 = 无显著域，1 = 正显著，-1 = 负显著反转候选）
- `backfill_range(fetch_day, start, end, *, max_rounds=40, round_pause=60.0) -> dict`：通用多轮扫描批量拉取（沉淀 Task 9 拉取模式，Phase 2-5 数据域复用）；遍历交易日，单日函数自行处理缓存命中/续拉，失败（抛异常或空 DataFrame）延后下一轮（轮间 round_pause），max_rounds 后返回 `{"total": int, "success": int, "failed": list[str]}`
- `fetch_fundamentals_range(start, end, *, max_rounds=40, round_pause=60.0) -> dict` / `fetch_moneyflow_range(start, end, *, max_rounds=40, round_pause=60.0) -> dict`：daily_basic / moneyflow 的断点续拉封装（复用 `fetch_fundamentals` / `fetch_moneyflow_by_date`），返回结构与 backfill_range 一致
