...
- **做法（路径②，2026-08-06 修正）**：对因子 IC 序列拟合 AR(1)（IC_t = c+φ·IC_{t-1}+ε）→ 残差中心化 → 打乱残差 → 无截距重建序列（起点 0，**均值归 0，H0：因子无效时 IC 均值=0**）→ 尾部 t_window 样本的 t 统计量零分布，重复 N 次（如 200 次）
- **逐因子独立**：不同因子的 IC 尺度/自相关不可比，**不跨因子混合**零分布（混合会膨胀 null_95 至 8.8，产生虚假高门槛）；每因子用自己的零分布 95 分位作门槛
- **保留 AR 结构**：φ 大（强自相关）→ 重建序列尾部均值漂移被放大 → 零分布更肥尾 → 门槛更严（自相关制造虚假显著性的幅度越大，要求越严）
- 只有 \|t\| > max(1, 因子自身 null_95) 的因子才允许入选（零分布分位数同时画成图上的逐因子参考线）
- **为何不是"打乱原值"**：t = mean/std×√n 对样本顺序不敏感，直接打乱等价于从边际分布不放回抽样，无法构造 H0；必须重建序列才能打破"残差随机"假设下 t 的分布
