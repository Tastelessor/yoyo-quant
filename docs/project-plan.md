...
| context (因子选择) | 🔲 路线图 | Phase C: factors/ops/synth.py（合成得分/信号/IC 权重）+ analysis/factor_synth.py::run_phase_c + yq factor clean-c（合成信号 vs 单因子回测对比）；Phase 挖掘 M1: factors/ops/layering.py + compute_ic_by_layer + data/moneyflow.py + factors/mining/（分层验证 + moneyflow 资金流族，见 factor-mining-phase1-plan.md） |
