"""GTJA earnings surprise strategy."""
...
@register_strategy("gtja_earnings_surprise")
class GTJAEarningsSurpriseStrategy(Strategy):
    ...
    def generate_signal(self, data, factors=None):
        return gtja_earnings_surprise_signal(
            data,
            self.rebalance,
            self.top_n,
            self.bottom_n,
...
def gtja_earnings_surprise_signal(
    df: pd.DataFrame,
    rebalance: int = 20,
    top_n: int = 5,
    bottom_n: int = 3,
        ...
        factor_df = pd.DataFrame(
            {
                "date": df["date"],
                "code": df["code"],
                "earnings_surprise": es.values,
                "earnings_acceleration": ea.values,
            }
        )
