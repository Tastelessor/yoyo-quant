...
DEFAULT_WEIGHTS = {"ma_slope_20d": 1.0, "macd_like": 1.0}
FACTOR_COLS = list(DEFAULT_WEIGHTS.keys())


@register_strategy("gtja_trend")
class GTJATrendStrategy(Strategy):
    name = "gtja_trend"
...
def gtja_trend_signal(
    ...
    if factors is not None and all(c in factors.columns for c in FACTOR_COLS):
        ...
        factor_df = pd.DataFrame({
            ...
            "ma_slope_20d": slope.values, "macd_like": macd.values,
        })
